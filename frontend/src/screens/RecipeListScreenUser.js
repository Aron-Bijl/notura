...
export default function RecipeListScreen(){
    ...
    useEffect(() => {
        const fetchData = async () => {
            try{
                const { data } = await axios.get(`/api/recipies/user?page=${page} `, {
    ...
    const createHandler = async () => {
        try{
            ...
            navigate(`/recipe/user/${data.recipe._id}`);
        }catch (err){
            toast.error(<p>{getError(err)}</p>);
            dispatch({
                type: 'CREATE_FAIL',
    ...
    const deleteHandler = async (recipe) => {
        if (window.confirm('Are you sure to delete?')){
            try{
                await axios.delete(`/api/recipies/user/${recipe._id}`, 
                {   
                    headers: { Authorization: `Bearer ${userInfo.token}` },
                });
                        ...
                        <table className='table'>
                            ...
                            {createdRecipe.map( (recipe) => (
                                <tr key={recipe._id}>
                                    ...
                                    <td className="action">
                                        <Button type="button"  buttonStyle="btn--outline" buttonSize="btn-medium"  onClick={() => navigate(`/recipe/user/${recipe._id}`)}>Edit</Button>&nbsp;
